 ...
 * @author Yong Ke
 */
public abstract class IteratorTag extends BodyTagSupport implements
		TryCatchFinally {
	...
	private void buildContext() {
		...
		if (o instanceof DPage) {
			context = new VWBContext(vwbcontext.getHttpRequest(),
					vwbcontext.getSite(), vwbcontext.getCommand(),
					(DPage) m_iterator.next());
		} else if (o instanceof CLBAttachment) {
			context = new VWBContext(vwbcontext.getHttpRequest(),
					vwbcontext.getSite(), vwbcontext.getCommand(),
					(CLBAttachment) m_iterator.next());
		} else {
			context = new VWBContext(vwbcontext.getHttpRequest(),
					vwbcontext.getSite(), vwbcontext.getCommand(),
					(Resource) m_iterator.next());
		}

		/*
		 * Push it to the iterator stack, and set the id.
		 */
		pageContext.setAttribute(VWBBaseTag.ATTR_CONTEXT, context,
