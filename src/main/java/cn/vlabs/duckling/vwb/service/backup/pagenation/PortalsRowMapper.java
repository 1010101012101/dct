 ...
 * @author zzb
 */
public class PortalsRowMapper extends AbstractConfigItemRowMapper implements ParameterizedRowMapper<ConfigItem> {
	...
	public ConfigItem mapRow(ResultSet rs, int row) throws SQLException {
		super.mapRow(rs, row);
		PortalItem portal = new PortalItem();
		portal.setId(id);
		portal.setParent(parent);
		portal.setTitle(title);
		portal.setBanner(banner);
		portal.setFooter(footer);
		portal.setLeftMenu(leftMenu);
		portal.setTopMenu(topMenu);
		portal.setTrail(trail);
		return portal;
	}
