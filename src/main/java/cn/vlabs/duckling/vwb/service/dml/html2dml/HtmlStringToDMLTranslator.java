...
public class HtmlStringToDMLTranslator {
	...
	public HtmlStringToDMLTranslator() {
	}
	...
	public String translate(String html, VWBContext vwbcontext)
			throws JDOMException, IOException {
		if (html.length() > 13) {
			if ((html.substring(html.length() - 13, html.length()))
					.equalsIgnoreCase("<p>&nbsp;</p>")) {
				html = html.substring(0, html.length() - 13);
			}
		}
		if (html.length() > 15) {
			String strremove = html
					.substring(html.length() - 15, html.length());
			String subStr1 = strremove.substring(0, 3);
			int ich = strremove.charAt(3);
			String subStr2 = strremove.substring(4);
			if ("<p>".equalsIgnoreCase(subStr1)
					&& "</p></body>".equalsIgnoreCase(subStr2) && ich == 160) {
