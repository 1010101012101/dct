...
public class ParseHtmlImg extends AbstractParseHtmlElement {
	 ...
	 * 判断是否需要将img转换成puglin
	 */
	private boolean isNotSpan2Plugin(Element e) {
		boolean reb = false;
		String strattibute = "";
	...
	private Map<String, String> parseArgs(String argstring) throws IOException {
	 ...
	 * </span> </span>
	 */
	private void printPlugin(Element element, Html2DmlEngine html2dmlengine) {
		html2dmlengine.getM_out().print("<D_plugin");
		...
		html2dmlengine.getM_out().println(" src=\"" + src + "\" >");
	...
	public void printAttribute(Element e, Html2DmlEngine html2dmlengine) {
		...
		String dosrc = html2dmlengine.findAttachment(
				e.getAttributeValue("src"), html2dmlengine);
		if (dosrc != null) {
	...
	public void printElement(Element e, Html2DmlEngine html2dmlengine) {
		...
		if (isNotSpan2Plugin(e)) {
			...
			if (e.getAttributeValue("alt") != null) {
				if (e.getAttributeValue("alt").equals("(info)"))
					attachtype = true;
			}
			if (!attachtype) {
				attachtype = false;
				html2dmlengine.getM_out().print("<img");
