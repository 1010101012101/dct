 ...
 * based on the current state of the user database, group manager, and built-in
 * role definitions. Creating a principal marked "unresolved" allows delayed
 * resolution, which enables principals to be resolved lazily during a later
 * access control check. Conceptuallly, UnresolvedPrincipal performs a function
 * similar to {@link java.security.UnresolvedPermission}.
 * 
 * @date Feb 3, 2010
 * @author zzb
 */
public final class UnresolvedPrincipal implements Principal {
	...
	/**
	 * An unresolved principal is equal to another unresolved principal if their
	 * names match.
	 * 
	 * @param obj
