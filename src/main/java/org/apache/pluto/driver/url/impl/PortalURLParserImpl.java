...
public class PortalURLParserImpl implements PortalURLParser {
    ...
    private static final String[] ESCAPED={"0x", "0xd"};
    ...
    public PortalURL parse(HttpServletRequest request) {
        ...
        URLParser parser = URLParser.getParser(request);
        String contextPath = parser.getContextPath();
        String servletName = parser.getServletName();
        ...
        String pathInfo = parser.getPathInfo();
...
//           Resource window definition: portalURL.setResourceWindow().
    ...
    public String toString(PortalURL portalURL) {
        ...
        if (portalURL.getRenderPath() != null) {
            String renderURL=portalURL.getRenderPath();
            if (renderURL.startsWith("/"))
                renderURL=renderURL.substring(1);
            buffer.append("/").append(renderURL);
        }
...
//原来允许中文页面，现在只允许添加英文页面，因此不需要再次转换了
    
    
//    private String decodeRnderPath(String path){
//        try {
//            path = decodeCharacters(path);
//            return URLDecoder.decode(path, "UTF-8");
//        } catch (UnsupportedEncodingException e) {
//            throw new RuntimeException(e);
//        }
//    }
//    private String encodeRenderPath(String render){
//        if (render.startsWith("/"))
//            render=render.substring(1);
//        try {
//            String encoded = URLEncoder.encode(render, "UTF-8");
//            return encodeCharacters(encoded);
//        } catch (UnsupportedEncodingException e) {
//            throw new RuntimeException(e);
//        }
//    }
    ...
    private String encodeMultiValues(String[] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            currentValue=StringUtils.replace(currentValue, ESCAPED[0], ESCAPED[1]);
...
//      // Decode the name into window ID and parameter name.
    ...
    private String decodeCharacters(String string) {
        ...
        string=StringUtils.replace(string, ESCAPED[1], ESCAPED[0]);
        return string;
    }
