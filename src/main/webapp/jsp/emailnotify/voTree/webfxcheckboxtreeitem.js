...
function WebFXCheckBoxTreeItem(sText, sType, sID, sPrefix, sAction, bChecked, eParent, sIcon, sOpenIcon) {
	this.base = WebFXTreeItem;
	this.base(sText, sPrefix, sAction, eParent, sIcon, sOpenIcon);
...
WebFXCheckBoxTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	str += "<img id=\"" + this.id + "-icon\" class=\"webfx-tree-icon\" src=\"" + ((webFXTreeHandler.behavior == 'classic' && this.open)?this.openIcon:this.icon) + "\" onclick=\"webFXTreeHandler.select(this);\"><a href=\"" + this.action + "\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" title=\""+this._sID+"\" onblur=\"webFXTreeHandler.blur(this);\">" + label + "</a></div>";
