...
FCKCutCopyCommand.prototype = {
	Execute : function() {
		...
		if (FCKBrowserInfo.IsIE) {
			...
			FCKTools.AddEventListener(FCK.EditorDocument.body, eventName, onEvent);
			//FCK.EditorDocument.body.attachEvent(eventName, onEvent);
			FCK.ExecuteNamedCommand(this.Name);
			FCKTools.RemoveEventListener(FCK.EditorDocument.body, eventName, onEvent);
...
FCKUpdateAttachCommand.prototype.GetState = function() {
	...
	var oAnchor = FCKSelection.MoveToAncestorNode("A");
	if (oAnchor) {
		if (oAnchor.getAttribute('href', 2)) {
			var href = oAnchor.getAttribute('href', 2);
...
TableRedrawCommand.prototype.GetState = function() {
	...
	var state = FCK_TRISTATE_DISABLED;
	var oTable = FCKSelection.HasAncestorNode("TABLE");
	if (oTable) {
		state = FCK_TRISTATE_OFF;
	}
	return state;
