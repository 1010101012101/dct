...
FCKFitWindow.prototype.Execute = function()
{
	...
	if ( !this.IsMaximized )
	{
		...
		// Switched to using FCKTools
		FCKTools.AddEventListener(eMainWindow, "resize", FCKFitWindow_Resize, true);
...
//			eMainWindow.addEventListener( 'resize', FCKFitWindow_Resize, true ) ;
	...
	else	// Resize to original size.
	{
		...
		FCKTools.RemoveEventListener(eMainWindow, "resize", FCKFitWindow_Resize, true);
//		if( FCKBrowserInfo.IsIE )
