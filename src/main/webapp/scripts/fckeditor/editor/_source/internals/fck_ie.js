...
FCK.InsertHtml = function(html) {
	...
	if (FCKSelection.GetType().toLowerCase()=="control")
		oSel.clear();
	...
	// Insert the HTML.
	  if (window.getSelection) {
	        // IE9 and non-IE
	        if (oSel.getRangeAt && oSel.rangeCount) {
	            ...
	            var el = oDoc.createElement("div");
	            var lastNode;
	            el.innerHTML = html;
	            var frag = oDoc.createDocumentFragment(), node, lastNode;
...
FCK._CheckIsPastingEnabled = function(returnContents) {
	...
	FCKTools.AddEventListener(document.body, "paste", FCK_CheckPasting_Listener);
