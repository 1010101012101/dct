...
FCK.InitializeBehaviors = function() {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener('mousedown', function(ev) {
			...
			if (element.nodeName.IEquals('IMG', 'HR', 'INPUT', 'TEXTAREA',
					'SELECT')) {
...
FCK.GetNamedCommandState = function(commandName) {
	switch (commandName) {
	case 'Unlink':
		return FCKSelection.HasAncestorNode('A') ? FCK_TRISTATE_OFF
...
FCK.InsertHtml = function(html) {
	...
	if (FCKBrowserInfo.IsGecko) {
		...
		html = '<span id="__fakeFCKRemove1__" style="display:none;">fakeFCKRemove</span>'
				+ html
...
FCK.InsertPlugin = function (html) {
	...
	try{
		oDoc.execCommand("inserthtml", false, html);
	}catch(e){}


	if (FCKBrowserInfo.IsGecko) {
		FCKDomTools.RemoveNode(oDoc.getElementById("__fakeFCKRemove1__"));
		FCKDomTools.RemoveNode(oDoc.getElementById("__fakeFCKRemove2__"));
	}
	this.Focus();
	var oRange = new FCKDomRange(this.EditorWindow);
	oRange.MoveToSelection();
	var bookMark = oRange.CreateBookmark();
	FCKDocumentProcessor.Process(oDoc);
	try {
