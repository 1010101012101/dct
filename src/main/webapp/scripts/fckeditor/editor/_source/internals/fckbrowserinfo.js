...
var FCKBrowserInfo = (function() {
	...
	var browserInfo = {
		...
		IsIE7 : !!s.match(/msie (\d+)/)
				&& (parseInt(s.match(/msie (\d+)/)[1], 10) >= 7)
				&& (parseInt(s.match(/msie (\d+)/)[1], 10) <= 8),
		IsIE9 : !!s.match(/msie (\d+)/)
				&& (parseInt(s.match(/msie (\d+)/)[1], 10) == 9),
		IsIE10 : !!s.match(/msie (\d+)/)
				&& (parseInt(s.match(/msie (\d+)/)[1], 10) == 10),
		IsIE11 : s.indexOf('trident') != -1
				&& (parseFloat(s.match(/trident\/(\d+)/)[1], 10) >= 7.0),
